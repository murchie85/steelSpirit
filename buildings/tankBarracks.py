...
class tankBarracks(parent):
	def __init__(self,_id,gui,x=None,y=None):
		super().__init__(gui)
		# MAIN OVERRIDES 
		self.id             = _id
		self.name           = 'tankBarracks'
		self.kind           = 'structure'
		self.images         = imageAnimateAdvanced(gui.tankBarracks,0.2)
		self.x,self.y       = 500,500
		if(x!=None): self.x = x
		if(y!=None): self.y = y
		self.w              = int(gui.tankBarracks[0].get_width())
		self.h              = int(gui.tankBarracks[0].get_height())
		
		self.chosenExplosionImg = gui.bigCloudyExplosion
		self.explosion          = imageAnimateAdvanced(self.chosenExplosionImg,0.1)
		
		# HIT IMAGE
		self.hitImage         = gui.tankBarracksHit
		self.hitAnimation     = imageAnimateAdvanced(self.hitImage,0.2)
		self.hitsTaken        = 0

		# DAMAGE IMAGES
		self.damagedBarracksImgs    = imageAnimateAdvanced(gui.tankBarracksDamaged,0.2)
		self.damagedBarracksImgsHit = imageAnimateAdvanced(gui.tankBarracksDamaged,0.2)

		# REMAINS IMAGE 
		self.remainsAnimation = imageAnimateAdvanced(gui.tankBarracksDestroyed,0.2)
