...
class attackBoat(parent):
	def __init__(self,_id,gui,x=None,y=None):
		super().__init__(gui)
		# MAIN OVERRIDES 
		self.id             = _id
		self.name           = 'attackBoat'
		self.kind           = 'boat'
		self.images         = imageAnimateAdvanced(gui.attackBoat,0.2)
		self.turretImage    = imageAnimateAdvanced(gui.attackBoatTurret,0.2)
		...
		# HIT IMAGE
		self.hitImage         = gui.attackBoatHit
		self.hitAnimation     = imageAnimateAdvanced(self.hitImage,0.2)
		self.turretHitImg     = imageAnimateAdvanced(gui.attackBoatTurretHit,0.2)
		self.hitsTaken        = 0
